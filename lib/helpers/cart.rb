module Cart
  ...
  def get_list_of_items_in_cart
    ...
    return order_items if session[:cart].nil? || session[:cart].empty? # skip if cart empty...
  ...
  def calculate_cart_items_cost
    total = 0
    return total if session[:cart].nil? || session[:cart].empty? # skip if cart empty...
    session[:cart].each do |item_id, quantity|
      info = {item_id: item_id, quantity: quantity}
      order_item = OrderItem.new(info)
  ...
  def calculate_cart_shipping
    weight = 0
    return weight if session[:cart].nil? || session[:cart].empty?  # skip if cart empty...
    session[:cart].each do |item_id, quantity|
      info = {item_id: item_id, quantity: quantity}
      order_item = OrderItem.new(info)
  ...
  def save_each_item_in_cart(order)
    return if session[:cart].nil? || session[:cart].empty?  # skip if cart empty...
    session[:cart].each do |item_id, quantity|
