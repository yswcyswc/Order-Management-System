...
class AddressScopeTest < ActiveSupport::TestCase
  context "Within context" do
    ...
    should "show that by_recipient places addresses in alphabetical order by recipient" do
    ...
    should "show that by_customer places addresses in alphabetical order by customer" do
      assert_equal ["Anthony Corletti", "Alex Egan", "Jeff Egan", "Zach Egan", "Ryan Flood", "Melanie Freeman"], Address.by_customer.all.map(&:recipient)
    end

    should "show that there are two active addresses and one inactive address" do
      assert_equal 5, Address.active.all.count
      assert_equal ["Alex Egan", "Anthony Corletti", "Jeff Egan", "Melanie Freeman", "Ryan Flood"], Address.active.all.map(&:recipient).sort 
      assert_equal 1, Address.inactive.all.count
